...
import BiNE.model.graph as graph
import random
from networkx.algorithms import bipartite as bi
import numpy as np
from BiNE.model.lsh import get_negs_by_lsh
from io import open
import os
import itertools
...
class GraphUtils(object):
    ...
    def construct_training_graph(self, G):
        edge_list_u_v = []
        edge_list_v_u = []
        for user, item in G.edges():
            rating = 1.
            if user not in self.edge_dict_u:
                self.edge_dict_u[user] = dict()
            if item not in self.edge_dict_v:
                self.edge_dict_v[item] = dict()
    ...
    def homogeneous_graph_random_walks_for_large_bipartite_graph_without_generating(self, datafile, percentage, maxT, minT):
        ...
        self.G_v, self.walks_v = self.get_random_walks_restart_for_large_bipartite_graph_without_generating(datafile, self.authority_v, percentage=percentage, maxT=maxT, minT=minT, node_type='m')
